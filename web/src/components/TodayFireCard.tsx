  ...
  if (!yanScore.hasCheckin) {
    return (
      <section className="rounded-2xl bg-white px-6 py-8" data-testid="today-fire-card">
        ...
        <div className="mt-2 text-3xl font-medium text-ink/40" data-testid="card-state">
          等待打卡
        </div>
        <p className="mt-2 text-sm text-ink/60 leading-relaxed">明早打卡后揭晓你的首份火分。</p>
        <Link
          href="/check-in/step1"
  ...
  if (!yanScore.result) {
    return (
      <section className="rounded-2xl bg-white px-6 py-8" data-testid="today-fire-card">
        ...
        <p className="mt-2 text-sm text-ink/60 leading-relaxed">再拍几餐 + 完整一天打卡后,火分会出现。</p>
      </section>
    );
  }
